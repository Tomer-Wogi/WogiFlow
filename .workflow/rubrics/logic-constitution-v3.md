...
**Amended**: 2026-04-15 — added P11.4 (Generative Edge-Case Taxonomy, 5 buckets) after the user observed mid-epic that architecturally-sound plans were still missing edge cases (workspace-mode sibling, pending-question orphaning, restart storm potential). Previous P11 was reactive — "did you address X?". P11.4 is generative — "here are 5 categories; acknowledge each one, or say N/A with reason."
...
### Sub-principle 11.4 — Generative edge-case taxonomy

**Added 2026-04-15 after the user observed mid-epic that architecturally-sound plans kept discovering edge cases post-hoc.** Prior P11/11.1/11.2/11.3 are REACTIVE — they critique what the plan says. P11.4 is GENERATIVE — for every mechanism, cross-reference against a fixed 5-bucket taxonomy and require explicit acknowledgment per bucket.

Plans pass P11.4 when they name each bucket and either address the concern OR assert "N/A, because..." in one line. Blank or silence on a bucket = FAIL.

**The 5 buckets** (deliberately trimmed from a draft of 12 to eliminate overlap with existing principles and avoid unnecessary filler):

**B1 — Interleaving / concurrency**
> What if this runs at the worst possible moment? What if it runs twice at the same time?
- TOCTOU: state changes between check and use
- Two instances of the same operation (two claude sessions, two worker processes, two `flow done` calls)
- Hook-in-hook race: operation A fires mid-way through operation B
- Real example caught by P11.4 review: concurrent WogiFlow sessions in the same repo both write the task-just-completed marker; the second restart-fire finds no marker

**B2 — Partial failure**
> If this is a multi-step operation, what if step 1 succeeds and step 2 fails?
- Can the system recover gracefully to a known-good state?
- Is there a "half-done" state that's worse than "not done"?
- Real example: `flow done` moves task → writes marker. If move succeeds and marker write fails, no restart fires but task is done. Acceptable. The reverse (marker written, move failed) would cause phantom restart — impossible in current code because marker is written AFTER move.

**B3 — Boundary counts**
> What if this runs 0 times? 1 time? 1000 times?
- Zero: first-ever invocation, empty queue, no prior history
...
- Does the mechanism LOG when it no-ops? When preconditions fail? When an unexpected state is encountered?
- Is there a `flow health`-equivalent command that surfaces when the feature is dormant?
- Is DEBUG-mode output sufficient to diagnose misbehavior?
- Real example: the self-improvement loop was dormant for 2+ months because `flow-correct.js` threw a ReferenceError on every invocation — silently. No one noticed. Observability gap.

**Evidence the Adversary demands for P11.4**:

For EACH bucket, the plan MUST produce one of:
- **Addressed**: a sentence or phrase describing how the concern is handled. Example: "B1 (interleaving): lock on `.workflow/state/ready.json` via `withLock()` serializes concurrent completions — verified in flow-done.js."
- **N/A with reason**: explicitly stated rationale. Example: "B4 (portability): out of scope. Windows users documented as unsupported in README. Fallback: features no-op cleanly when wrapper isn't detected."
- **Known limitation accepted**: explicit acceptance. Example: "B3 (many): bulk-storm acknowledged. Acceptable tradeoff: context-savings compound across N restarts, so net-gain persists even at N=100. Documented in spec."

Blank, hand-wavy ("we'll figure it out later"), or silent buckets = FAIL.

**Why exactly 5, not more**:

Earlier drafts had 12 buckets. On review:
- TOCTOU, concurrency, mid-operation interleaving all collapse into B1
- Zero-case, N-case, cumulative effect all collapse into B3
- "Hostile input" is generic code-review territory, not mechanism-specific
- "Sibling features" is P11.3, don't duplicate
- "Reversibility" is principle 10, don't duplicate
- "Silent-attacker" is broader WogiFlow threat model, not mechanism-specific

What remains are 5 categories that are: (a) unique, (b) load-bearing for real past failures, (c) generative rather than validative, (d) low token-cost to address in plans (~50-100 words total vs architect essay).
...
Principles 1, 2, 5, 7, 8, 10, 11 ALWAYS run. Even in fully degraded mode (no intent artifacts), the Adversary provides 7-principle coverage. Principle 11 (including 11.1-11.4) never skips — capability claims, self-grounded rule compliance, sibling-surface integration, and edge-case taxonomy must always be verified regardless of intent-artifact availability.
