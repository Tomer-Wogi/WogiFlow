# WogiFlow for Kimi CLI

Kimi has **soft enforcement only** (advisory). WogiFlow cannot block operations in Kimi because it lacks pre-operation hooks.

## Setup

1. Install WogiFlow: `npm install wogiflow`
2. Initialize: `npx flow onboard`
3. Sync bridge: `npx flow bridge sync kimi`
...
Kimi provides **soft enforcement only**:

- No pre-operation hooks
- Rules in AGENTS.md are advisory
- LLM must self-enforce the workflow

**IMPORTANT**: Because Kimi cannot block operations, you rely on the LLM following the instructions in AGENTS.md. For guaranteed enforcement, use Claude Code, Gemini CLI, or OpenCode instead.
...
These protocols may enable future enforcement capabilities.
...
Because Kimi lacks hooks, the AGENTS.md file includes detailed self-enforcement instructions:
...
- **MCP/ACP support**: Protocol support for future integrations
...
- You need MCP/ACP protocol support
