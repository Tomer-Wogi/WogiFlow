...
### Features in 2.1.140 → 2.1.158 (consolidated)

Catch-up pass for the 2.1.14x–2.1.15x range, focused on the WogiFlow-relevant surface. Most items in these releases are Claude Code TUI/terminal/IDE/background-session internals (terminal rendering, clipboard/paste, WSL image paste, `--resume` picker, `/model` picker hint, fast-mode indicator, sandbox banners, managed-settings dialog, image-crash fix, long-conversation render perf) with **no WogiFlow code surface** — they benefit sessions passively and need no action.

- **Opus 4.8 (2.1.154) + Opus 4.8 auto/effort defaults (2.1.154); thinking-block API-error fix (2.1.154)** — Opus 4.8 now defaults to `high` effort, adds `/effort xhigh`, and fast mode runs at 2x rate / 2.5x speed. **Impact on WogiFlow (acted on, wf-27026d0b)**: the model registry now knows Opus 4.8 — added `claude-opus-4-8` to `scripts/flow-providers.js` (capability map + interactive picker, now the recommended "latest") and to the `claude-opus` alias list in `scripts/flow-model-adapter.js` so per-model prompt adaptation, hybrid-mode routing, and peer-review recognize it. The CC-side fix for Opus 4.8 thinking blocks being modified (→ API 400s) is automatic. Note: WogiFlow's advisory effort mapping in `wogi-start.md` (L2→`medium`) intentionally still differs from CC's new `high` default — see the 2.1.117 note above.
- **`.claude/skills/` directories now auto-loaded, no marketplace required; `claude plugin init <name>` scaffolds there (2.1.157)** — **Impact on WogiFlow**: positive and automatic. WogiFlow already installs its skills into `.claude/skills/` via `lib/skill-registry.js`, so they auto-load with no change. This is the **CC skills layer** and is distinct from WogiFlow's **MCP/plugin registry** (`/wogi-register` → `.workflow/state/plugin-registry.json`), which routes at the WogiFlow semantic layer and is unaffected. No code change.
- **"Workflow keyword trigger" `/config` setting; backspace dismisses a workflow trigger (2.1.157)** — typing the bare word "workflow" in a free-form prompt can trigger CC's dynamic-workflow feature. **Impact on WogiFlow**: advisory only. WogiFlow interactions are driven by explicit `/wogi-*` slash commands, so the trigger rarely fires in practice; but users typing free-form prompts like "start the workflow" / "check workflow status" may hit it. Cure: prefer `/wogi-*` commands, or disable via `/config` → "Workflow keyword trigger". No code change.
- **Claude-managed worktrees left unlocked when the agent finishes; `.claude/worktrees/` 30-day retention sweep; `--worktree` returning the linked worktree path not the canonical root (2.1.157)** — **Impact on WogiFlow**: none, confirmed. WogiFlow never `git worktree lock`s its worktrees (it creates them in an OS-temp dir, not `.claude/worktrees/`, and prunes them on its own ~24h cycle — `scripts/flow-worktree.js`), so "left unlocked" and the 30-day `.claude/worktrees/` sweep are fully separate from WogiFlow's cleanup — no collision. WogiFlow's worktree-stable state resolution uses `git rev-parse --git-common-dir` (`scripts/flow-paths.js`), not `--show-toplevel`/`cwd`, so the `--worktree` canonical-root fix doesn't change any WogiFlow assumption. No code change.
- **Auto mode on Bedrock/Vertex/Foundry for Opus 4.7+4.8 via `CLAUDE_CODE_ENABLE_AUTO_MODE=1` (2.1.158)** — **Impact on WogiFlow**: orthogonal, documented for clarity. CC "auto mode" (model-level agentic autonomy) is **not** WogiFlow's "Autonomous Walk-Away Mode" (`flow-autonomous-mode.js`, an NL-triggered task-queue drain). They operate at different layers and don't interact; setting `CLAUDE_CODE_ENABLE_AUTO_MODE=1` does not touch WogiFlow's queue. No code change.
- **`agent` field in settings.json honored for dispatched sessions + `--agent <name>` (2.1.157)**; **`tool_decision` telemetry adds `tool_parameters` under `OTEL_LOG_TOOL_DETAILS=1` (2.1.157)**; **`EnterWorktree` can switch managed worktrees mid-session (2.1.157)** — **Impact on WogiFlow**: none. WogiFlow sets no top-level `agent` settings field (it routes model via the Agent tool `model` param), consumes no CC OTel `tool_decision` stream (its gate telemetry is a separate JSONL — `flow-gate-telemetry.js`), and doesn't drive `EnterWorktree`. Automatic where relevant.
