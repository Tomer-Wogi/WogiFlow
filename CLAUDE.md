...
This applies to ALL message types — implementation, questions, conversations, exploration, research, operational requests. You do NOT handle requests directly. If you think "this is just a question, I can handle it directly" — that thought is the exact bypass this rule exists to prevent.
...
3. Update `function-map.md` if utilities created/deleted/renamed — run `node scripts/flow-function-index.js scan`
4. Update `api-map.md` if endpoints created/deleted/renamed — run `node scripts/flow-api-index.js scan`
5. Update other active registry maps if relevant — run `node scripts/flow-registry-manager.js scan`
...
**Required:** Web search before claiming capabilities. Exhaustive search before saying "doesn't exist."

**FORBIDDEN** — never say "X is not supported" or "there is no Y". Instead:
```
I searched [sources] and found no evidence of X. My search may be incomplete.
Check official docs at [URL], feature may have a different name.
```
...
Last synced: 2026-02-23T19:27:33.775Z
