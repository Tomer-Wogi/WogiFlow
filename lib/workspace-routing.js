...
async function dispatchCrossRepoPlan(workspaceRoot, createdTasks, opts = {}) {
  ...
  if (opts.parallel) {
    ...
    for (const phase of sortedPhases) {
      ...
      // Note: In a full implementation, we'd wait for phase completion
      // before starting the next phase. For now, we dispatch and let
      // the manager monitor via waitForCompletion().
    }
  }
