...
function checkWorkspaceHealth(workspaceRoot, manifest) {
  ...
  for (const [name, member] of Object.entries(manifest.members)) {
    ...
    if (!fs.existsSync(memberPath)) {
      issues.push({ severity: 'error', check: 'member-exists', message: `Member '${name}' path does not exist: ${memberPath}` });
      checks.failed++;
    } else {
      checks.passed++;
    }
...
function exportForDashboard(workspaceRoot, manifest) {
  ...
  return {
    ...
    messages: {
      total: readMessages(workspaceRoot).length,
      pending: readMessages(workspaceRoot, { status: 'pending' }).length
    },
