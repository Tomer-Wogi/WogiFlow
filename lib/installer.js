...
function createWorkflowStructure(projectRoot, config) {
  ...
  // Create config.json with comprehensive defaults
  ...
  const configContent = {
    ...
    qualityGates: {
      feature: {
        require: ["loopComplete", "requestLogEntry", "appMapUpdate"]
      },
      bugfix: {
        require: ["loopComplete", "requestLogEntry"]
      },
      refactor: {
        require: ["loopComplete", "requestLogEntry", "noNewFeatures"]
      }
    },
    standardsCompliance: {
      enabled: true,
    ...
    componentIndex: {
      enabled: true,
      scanDirs: ["src/components", "components", "src/ui"]
    },
    functionRegistry: {
      enabled: true,
      scanDirs: ["src/utils", "src/helpers", "src/lib", "utils", "helpers", "lib"]
    },
    apiRegistry: {
      enabled: true,
      scanDirs: ["src/api", "src/routes", "api", "routes", "src/pages/api"]
    },
    registries: [
    ...
    community: {
      ...
      _comment_community: "Opt-in anonymous community knowledge sharing. Enable to share/receive model intelligence, error strategies, and universal patterns. No code, paths, or personal data ever leaves the machine."
    }
  };
