...
function getDefaultConfig(overrides = {}) {
  const base = {
    ...
    cli: overrides.cli !== undefined ? overrides.cli : 'claude',
    enforcement: {
      strictMode: true,
      requireTasks: true,
      requireApproval: true,
      taskSizeThresholds: {
        L3_subtask: { maxFiles: 1 },
        L2_task: { maxFiles: 5, maxCriteria: 3 },
        L1_story: { maxFiles: 15, maxCriteria: 10 },
        L0_epic: { minFiles: 15 }
      }
    },
    releaseChannel: overrides.releaseChannel !== undefined ? overrides.releaseChannel : 'stable',
    autoUpdate: { enabled: true, notifyOnly: true },
    tasks: {
      maxIterations: 10,
      blockExitUntilComplete: true,
      specVerification: true,
    ...
    qualityGates: {
      feature: { require: ["loopComplete", "requestLogEntry", "appMapUpdate"] },
      bugfix: { require: ["loopComplete", "requestLogEntry"] },
      refactor: { require: ["loopComplete", "requestLogEntry", "noNewFeatures"] }
    },
    standardsCompliance: {
      enabled: true,
      mode: "block",
      ...
      learning: { enabled: true, promotionThreshold: 3, autoSyncRules: true }
    },
    semanticMatching: {
      thresholds: { definiteMatch: 90, likelyMatch: 70, possibleMatch: 50 },
      weights: { string: 0.3, semantic: 0.7 },
    ...
    validation: {
      afterFileEdit: { enabled: true, commands: ["npx tsc --noEmit"] },
      beforeCommit: { enabled: true, commands: ["npm run lint", "npm run typecheck"] }
    },
    commits: {
    ...
    smartCompaction: {
      ...
      estimation: { perFile: 0.02, perCriterion: 0.03, refactorBuffer: 0.10 }
    },
    context: {
      compaction: {
        safeContextPercentage: 85,
        warningPercentage: 75,
        autoCleanupReadyJson: true,
        maxRecentlyCompleted: 10
      }
    },
    review: {
      enabled: true,
      mode: "parallel",
      multiPassThreshold: 5,
      agents: {
        codeLogic: { enabled: true },
    ...
    planMode: {
      ...
      approvalGate: { enabled: true, minTaskLevel: "L1" }
    },
    research: {
    ...
    clarifyingQuestions: {
      ...
      smallTaskThreshold: 2
    },
    specificationMode: {
      enabled: true,
      minTaskLevel: "L2",
      generateSpecs: true,
      needsClarification: { enabled: true, blockImplementation: true }
    },
    storyDecomposition: { autoDetect: true, autoDecompose: false },
    componentIndex: { enabled: true, scanDirs: ["src/components", "components", "src/ui"] },
    functionRegistry: { enabled: true, scanDirs: ["src/utils", "src/helpers", "src/lib", "utils", "helpers", "lib"] },
    apiRegistry: { enabled: true, scanDirs: ["src/api", "src/routes", "api", "routes", "src/pages/api"] },
    registries: [
      { id: "components", enabled: true, activateWhen: "frontend" },
      { id: "functions", enabled: true, activateWhen: "always" },
      { id: "apis", enabled: true, activateWhen: "always" },
      { id: "schemas", enabled: "auto", activateWhen: "orm" },
      { id: "services", enabled: "auto", activateWhen: "backend" }
    ],
    sessionLearning: { enabled: true, autoApplyThreshold: 90, minOccurrences: 2 },
    morningBriefing: { enabled: true, showTechDebt: true, showSessionContext: true },
    techDebt: { enabled: true, debtBudget: { maxItems: 50 }, autoFix: { enabled: false } },
    longInputGate: {
      enabled: true,
      charThreshold: 3000,
      lineThreshold: 60,
...
function createWorkflowStructure(projectRoot, config) {
  ...
  const configContent = getDefaultConfig({
    ...
    cli: config.cli,
    enforcement: { strictMode: config.strictMode },
...
module.exports = { init, getDefaultConfig, deepMerge };
