...
function getDefaultConfig(overrides = {}) {
  const base = {
    ...
    scripts: { lint: 'npx eslint [file] --fix', typecheck: 'npx tsc --noEmit', test: null, build: null, fix: null, coverage: null },
    requireApproval: [],
    enforcement: {
 ...
 *
 * @param {string} projectRoot - Project root directory
 * @returns {Object} Detected scripts mapped to canonical keys (e.g., { typecheck: 'npm run type-check', lint: 'npm run lint' })
 */
  ...
  for (const [canonicalKey, variants] of Object.entries(SCRIPT_MAPPINGS)) {
    for (const variant of variants) {
      if (Object.prototype.hasOwnProperty.call(scripts, variant)) {
        detected[canonicalKey] = `npm run ${variant}`;
...
async function init(args) {
  ...
  if (!detectedScripts.typecheck) {
    ...
    if (tsMode === 'project-references') {
      config.scripts.typecheck = 'npx tsc --build --force';
    }
  }
