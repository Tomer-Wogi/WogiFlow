...
const { buildIntegrationMap, normalizeForMatching, matchScore, detectTypeDrift } = require('./workspace-contracts');
const { detectContractDrift, getCascadeTargets, buildDependencyGraph, checkWorkspaceHealth } = require('./workspace-intelligence');
const { createMessage, saveMessage, getUnreadMessages, generateChangeNotifications } = require('./workspace-messages');
const { checkWorkerHealth } = require('./workspace-routing');
...
function workspaceActive(cwd) {
  ...
  if (envRoot) {
    const envConfig = path.join(envRoot, WORKSPACE_CONFIG_FILE);
    if (fs.existsSync(envConfig)) {
      return { active: true, root: envRoot, configPath: envConfig };
    }
  }
