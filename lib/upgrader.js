...
function runMigrations(projectRoot, fromVersion, toVersion, dryRun) {
  ...
  const migrations = [
    ...
    },
    {
      from: '1.0.0',
      to: '1.6.0',
      name: 'Enable longInputGate by default',
      run: (root) => {
        const configPath = path.join(root, '.workflow', 'config.json');
        if (!fs.existsSync(configPath)) return;
        try {
          const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
          if (!config.longInputGate) {
            config.longInputGate = {
              enabled: true,
              charThreshold: 3000,
              lineThreshold: 60,
              smartDefault: true,
            ...
            config.longInputGate.enabled = true;
            if (!config.longInputGate.lineThreshold || config.longInputGate.lineThreshold < 60) {
              config.longInputGate.lineThreshold = 60;
            }
          }
          fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
        } catch (err) {
          console.error(`  Warning: Could not update longInputGate config: ${err.message}`);
        }
      }
    }
