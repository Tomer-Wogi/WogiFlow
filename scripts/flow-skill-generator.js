...
function getPrebuiltSkillsDir() {
  // __dirname is scripts/, package root is one level up
...
function getPrebuiltSkillPath(techValue) {
  const skillId = techValue.toLowerCase().replace(/[^a-z0-9]/g, '-');
...
function copyPrebuiltSkill(prebuiltPath, projectRoot, skillId) {
  ...
  // Copy all files from pre-built template
  const entries = fs.readdirSync(prebuiltPath, { withFileTypes: true });
  for (const entry of entries) {
    const srcPath = path.join(prebuiltPath, entry.name);
    ...
    if (entry.isDirectory()) {
      ...
      for (const subEntry of subEntries) {
        fs.copyFileSync(path.join(srcPath, subEntry), path.join(destPath, subEntry));
      }
    } else {
  ...
  // Determine type from skill.md frontmatter
  let type = 'library';
  try {
    const skillMd = fs.readFileSync(path.join(destDir, 'skill.md'), 'utf8');
    const typeMatch = skillMd.match(/^type:\s*(.+)$/m);
    if (typeMatch) {
...
libraryVersion: ${docs?.libraryVersion || 'unknown'}
lastRefreshed: ${new Date().toISOString()}
generated: ${new Date().toISOString()}
incomplete: ${docs ? 'false' : 'true'}
learningCount: 0
successRate: null
---
...
- \`knowledge/conventions.md\` — Team conventions (highest priority)
- \`knowledge/anti-patterns.md\` — Mistakes to avoid
- \`knowledge/patterns.md\` — Best practices
- \`knowledge/learnings.md\` — Accumulated knowledge from work
- \`knowledge/library-reference.md\` — Context7 API docs (supplementary)
...
/**
 * Generate knowledge/conventions.md — Team-specific conventions (highest priority content layer)
 * Distinct from rules/conventions.md which holds generic code-style conventions.
 * This file holds project-specific conventions detected during onboarding or added by the team.
 */
function generateTeamConventionsMd(tech, docs) {
  return `# ${tech.label} Team Conventions

Project-specific conventions for ${tech.label} in this codebase.
These take precedence over library defaults when there is a conflict.

## Import Patterns

${docs?.importPatterns || `*Will be populated during onboarding by analyzing existing code.*`}

## File Structure Conventions

${docs?.fileStructure || `*Will be populated during onboarding by analyzing project layout.*`}

## Error Handling Style

${docs?.errorHandling || `*Will be populated during onboarding by analyzing error patterns.*`}

## Naming Patterns

${docs?.namingPatterns || `*Will be populated during onboarding by analyzing naming conventions.*`}

---
*source: ${docs?.conventionSource || 'onboard-detected'}*
*Auto-generated — edit freely to capture team preferences*
`;
}

/**
 * Generate knowledge/library-reference.md — Context7-sourced API docs (lowest priority content layer)
 * This is supplementary reference material fetched from Context7 MCP.
 * When token budget is tight, this file is the first to be trimmed.
 */
function generateLibraryReferenceMd(tech, docs) {
  if (docs?.libraryReference) {
    return `# ${tech.label} Library Reference

<!-- Library Reference (supplementary) — fetched via Context7 MCP -->
<!-- This content is auto-generated. Team conventions in conventions.md take precedence. -->

${docs.libraryReference}
...
<!-- Library Reference (supplementary) — will be populated via Context7 MCP -->
<!-- Run /wogi-setup-stack --fetch-docs or /wogi-rescan to populate -->

*No library documentation fetched yet.*

To populate this file:
1. Ensure Context7 MCP is available
2. Run \`/wogi-setup-stack --fetch-docs\` or \`/wogi-rescan\`
3. Context7 will fetch API docs for ${tech.label} (${tech.context7 || 'no Context7 ID'})
...
- \`knowledge/conventions.md\` — Team conventions (highest priority)
- \`knowledge/anti-patterns.md\` — Mistakes to avoid
- \`knowledge/patterns.md\` — Core ${tech.label} patterns
- \`knowledge/learnings.md\` — Accumulated knowledge
- \`knowledge/library-reference.md\` — Context7 API docs (supplementary)
...
- \`knowledge/conventions.md\` — Team conventions (highest priority)
- \`knowledge/anti-patterns.md\` — Mistakes to avoid
- \`knowledge/patterns.md\` — Detailed patterns
- \`knowledge/learnings.md\` — Accumulated knowledge
- \`knowledge/library-reference.md\` — Context7 API docs (supplementary)
  ...
  for (const [skillId, { tech, type }] of techMap) {
    ...
    skills[skillId] = {
      ...
      sections: {
        ...
        libraryReference: 'knowledge/library-reference.md',
        rulesConventions: 'rules/conventions.md'
      }
    };
  }
...
async function writeSkillFiles(tech, docs, projectRoot, skillContext = {}) {
  ...
  // Core files for all skills — content layers in priority order
  const files = [
    { path: 'skill.md', content: skillMdContent },
    // Team conventions — highest priority knowledge layer (distinct from rules/conventions.md)
    { path: 'knowledge/conventions.md', content: generateTeamConventionsMd(tech, docs) },
    ...
    // Library reference — lowest priority, supplementary Context7 docs
    { path: 'knowledge/library-reference.md', content: generateLibraryReferenceMd(tech, docs) }
  ];

  // Framework skills also get rules/conventions.md (legacy location, code-style conventions)
  if (type === 'framework') {
...
async function enhanceSkillWithDocs(skillId, docs) {
  ...
  // Update conventions.md (rules/ location - generic code-style conventions)
  ...
  // Update library-reference.md with fetched Context7 docs
  if (parsed.patterns || parsed.antiPatterns || parsed.examples) {
    const libraryRefPath = path.join(skillDir, 'knowledge', 'library-reference.md');
    const sections = [];
    if (parsed.patterns) sections.push(`## Patterns\n\n${parsed.patterns}`);
  ...
  // Update skill.md frontmatter: set incomplete to false and update lastRefreshed
  const skillMdPath = path.join(skillDir, 'skill.md');
  if (fs.existsSync(skillMdPath)) {
    let skillMd = fs.readFileSync(skillMdPath, 'utf8');
    skillMd = skillMd.replace(/incomplete:\s*(true|false)/, 'incomplete: false');
    skillMd = skillMd.replace(/lastRefreshed:\s*\S+/, `lastRefreshed: ${new Date().toISOString()}`);
...
module.exports = {
  ...
  generateTeamConventionsMd,
  generateLibraryReferenceMd,
  categorizeLibrary,
