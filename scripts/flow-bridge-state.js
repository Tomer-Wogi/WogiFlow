...
async function autoSyncBridge(cliType = 'claude-code', options = {}) {
  ...
  try {
    ...
    // Store content hash for drift detection
    storeClaudeMdHash();
...
/**
 * Check if CLAUDE.md content has drifted from what the template would generate.
 * Used by SessionStart to detect manual edits.
 *
 * @returns {{ drifted: boolean, reason: string }}
 */
function checkClaudeMdDrift() {
  try {
    const claudeMdPath = path.join(PROJECT_ROOT, 'CLAUDE.md');
    if (!fs.existsSync(claudeMdPath)) {
      return { drifted: false, reason: 'file-missing' };
    }

    const content = fs.readFileSync(claudeMdPath, 'utf-8');

    // Check if the generation marker is present
    if (!content.includes('Generated by CLI Bridge')) {
      return { drifted: true, reason: 'marker-missing' };
    }

    // Hash the current CLAUDE.md content (excluding the timestamp line which always changes)
    const contentWithoutTimestamp = content.replace(/Last synced: .+/g, 'Last synced: TIMESTAMP');
    const currentHash = crypto.createHash('md5').update(contentWithoutTimestamp).digest('hex');

    // Compare against stored hash from last sync
    const state = readSyncState();
    const storedHash = state.syncs?.['claude-code']?.contentHash;

    if (!storedHash) {
      // No stored hash — first time, store it
      return { drifted: false, reason: 'no-baseline' };
    }

    if (currentHash !== storedHash) {
      return { drifted: true, reason: 'content-changed' };
    }
...
/**
 * Store a content hash of CLAUDE.md after sync for drift comparison.
 * Called after successful bridge sync.
 */
function storeClaudeMdHash() {
  try {
    const claudeMdPath = path.join(PROJECT_ROOT, 'CLAUDE.md');
    ...
    const content = fs.readFileSync(claudeMdPath, 'utf-8');
    const contentWithoutTimestamp = content.replace(/Last synced: .+/g, 'Last synced: TIMESTAMP');
    const contentHash = crypto.createHash('md5').update(contentWithoutTimestamp).digest('hex');
